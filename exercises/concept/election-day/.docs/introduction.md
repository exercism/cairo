...
If we create a map and call this function, the changes the function made to the map persist after the function ends. This is a similar behaviour we get if we were using a pointer, but note how, on this example, we are not using any referencing/dereferencing or any of the pointer syntax:
